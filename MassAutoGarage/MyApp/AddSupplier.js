    ...
    if (window.FormData !== undefined) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                debugger;
                if (result == "File size Should Be UpTo 7 KB !" || result == "PDF File size Should Be UpTo 5 MB !") {
...
$('#Photourl').change(function () {
    ...
    if (window.FormData !== undefined) {
        ...
        $.ajax({
            url: '/SupplierMaster/UploadPhoto',
            type: "POST",
            contentType: false, // Not to set any content header
            processData: false, // Not to process data
            data: fileData,
...
$('#Signatureurl').change(function () {
    debugger;
    // Checking whether FormData is available in browser
    if (window.FormData !== undefined) {

        var fileUpload = $("#Signatureurl").get(0);
        var files = fileUpload.files;
...
$("#btnSave").click(function () {
    ...
    var _hdfsign = $("#hdfsign").val();
    var _hdfphoto = $("#hdfphoto").val();
...
$("#btnWhatsapp").click(function () {
    ...
    var Message = $("#txtWhatsappMess").val();

    if (MobileNumber != "" && Message != "") {

        $.ajax({
            type: 'GET',
            url: '/SupplierMaster/SendWhatsappMessage',
            data: { Mobile: MobileNumber, whatsAppMessage: Message },
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
            success: function (data) {
                debugger;
                if (data != "") {
                    ...
                    /* var url = "https://api.whatsapp.com/send?phone=" + MobileNumber + "&text=" + WhatsappMessage;*/
                } 
            }
        });
        ...
        if (MobileNumber == "") {
            ...
            swal("Warning", "Mobile No. is Required !", "warning");
            $("#Mobile").focus();
            return false;
        ...
        if (Message == "") {
            ...
            swal("Warning", "Whatsapp Message is Required !", "warning");
            $("#txtWhatsappMess").focus();
            return false;
